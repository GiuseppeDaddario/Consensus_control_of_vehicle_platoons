...
class Simulation:
    def __init__(self, dt=0.01, mode="interactive", total_time=60.0, P_init=None, V_init=None, A_init=None, h=1.0, c1=0.8, c2=0.2, leader_profile=None, scenario_name=None):
        self.dt = dt
        ...
        self.leader = Vehicle(id="Leader", start_pos=P_init[0], start_vel=V_init[0], length=5.0)
        self.leader.a = A_init[0]
    ...
    def _scenario_command(self):
        if self.leader_profile is None:
            self.a_ref = 0.0
        else:
            self.a_ref = self.leader_profile(self.t)

        self.v_ref += self.a_ref * self.dt
        self.p_ref += self.v_ref * self.dt
        r_star = np.array([self.p_ref, self.v_ref, self.a_ref])

        return self.controller.compute_virtual_leader_u(r_star, self.leader.get_state())
    ...
    def step(self):
        ...
        safe_leader_cmd = self.controller.apply_cbf(nominal_leader_u, self.leader, None)
        self.leader.step(self.dt, safe_u=safe_leader_cmd)

        u_dots = self.controller.compute_nominal_commands(self.vehicles, self.leader)

        for i in range(self.num_vehicles):
            curr_veh = self.vehicles[i]
            prev_veh = self.leader if i == 0 else self.vehicles[i - 1]
        ...
        self.last_leader_cmd = safe_leader_cmd
        self._log()
        self.t += self.dt
    ...
    def _log(self):
        ...
        self.log["v"][0].append(self.leader.v)
        self.log["a"][0].append(self.leader.a)
        self.log["u"][0].append(self.leader.u)
